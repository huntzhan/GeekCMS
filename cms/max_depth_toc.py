...
def order_toc_list(toc_list, max_depth):
    ...
    def remove_nodes(node_list, max_depth):
        if len(node_list) == 0:
            return

        for children in node_list:
            if children['level'] == max_depth:
                children['children'] = []
            else:
                remove_nodes(children['children'], max_depth)
        
    if max_depth is not None:
        remove_nodes(ordered_list, max_depth)
...
class TocTreeprocessor(Treeprocessor):
    ...
    def run(self, doc):
        ...
        marker_pattern = re.compile(self.config["marker"])
        max_depth = None
        ...
        for (p, c) in self.iterparent(doc):
            ...
            if c.text:
                m = marker_pattern.match(c.text.strip())
        ...
        toc_list_nested = order_toc_list(toc_list, max_depth)
...
class MaxDepthTocExtension(Extension):
    
    TreeProcessorClass = TocTreeprocessor
    
    def __init__(self, configs=[]):
        self.config = { "marker" : ["\[TOC-(\d+?)\]", 
    ...
    def extendMarkdown(self, md, md_globals):
        ...
        md.treeprocessors.add("max_depth_toc", tocext, "_end")


def makeExtension(configs={}):
    return MaxDepthTocExtension(configs=configs)
